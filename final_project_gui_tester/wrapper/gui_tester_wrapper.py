"""Wrapper that assembles and runs the GUI tester without modifying demo code."""
...
from final_project_gui_tester.tools.gui_testing_report_tool import build_tool as build_report_tool
from final_project_gui_tester.tools.restricted_playwright_computer_use import build_tool as build_computer_use_tool

PROJECT_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = PROJECT_ROOT.parent
DEMO_CUSTOM_AGENT_DIR = REPO_ROOT / "demos" / "comp_use" / "custom_agent"

if str(DEMO_CUSTOM_AGENT_DIR) not in sys.path:
    sys.path.insert(0, str(DEMO_CUSTOM_AGENT_DIR))
...
def run_gui_tester_session(
    ...
    config_file = (
        ...
        if config_path
        else PROJECT_ROOT / "config" / "default_tester_config.json"
    )
