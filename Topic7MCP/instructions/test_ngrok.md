Save this as `test_ngrok.ps1` and run it from PowerShell.
...
* It assumes `ngrok` and `python` are both available on your PATH.
* It binds the test server to `127.0.0.1`, so it only exposes the tunnel, not your whole machine directly.
* Unlike `python -m http.server`, this returns a tiny JSON response, which is closer to your professor’s original Bash test.

If you want, I can also make you a **shorter, cleaner student version** with less logging and fewer checks.
