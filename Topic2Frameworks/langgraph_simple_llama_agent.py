...
def create_graph(llm, qwen_llm):
    ...
    # NODE: fanout
    # =========================================================================
    # Dispatcher node that acts as a named fork point. It reads `user_input`
    # and returns nothing (state unchanged) — its presence lets us add two
    # outgoing edges to run both model nodes in parallel.
    def fanout(state: AgentState) -> dict:
        if state.get("verbose", False):
            print(f"[TRACE] fanout received user_input='{state.get('user_input','')}'")
        return {}
    ...
    def print_both(state: AgentState) -> dict:
        ...
        if not state.get("llama_response") or not state.get("qwen_response"):
            if state.get("verbose", False):
                print("[TRACE] print_both missing one or both responses; skipping print")
            return {}
    ...
    graph_builder.add_node("fanout", fanout)
    ...
    # 3. fanout -> call_llama AND fanout -> call_qwen (parallel branches)
    graph_builder.add_edge("fanout", "call_llama")
    graph_builder.add_edge("fanout", "call_qwen")
